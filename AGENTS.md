...
## Testing Guidelines
Add unit tests alongside the relevant module and keep integration-style checks in `tests/` if they grow beyond fixtures. The `nightly` feature requires a nightly Rust toolchain, so use `cargo test --features rodio,hound` on stable and `cargo +nightly test --features nightly` separately. Property or fuzz tests belong under `fuzz/fuzz_targets/`; reuse fixture audio for deterministic assertions. Target at least the existing coverage footprint—any new surface should include decoding or encoding assertions.
